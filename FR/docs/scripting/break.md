...
`for i in range(10):
    break
print(i)`
Cela affiche `0` parce que `i` est `0` à la première itération de la boucle, puis l'instruction break termine la boucle.
...
`for i in range(10):
    for j in range(10):
        break
        print("cela n'est jamais imprimé")
    print("cela est imprimé 10 fois")`
