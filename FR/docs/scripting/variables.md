...
Le côté gauche de l'opérateur est le nom de la variable. Vous pouvez lui donner n'importe quel nom souhaité.
Le côté droit est une expression dont la valeur résultante sera stockée dans la variable.

Déclarez une variable nommée `a` et stockez-y la valeur `5` :
`a = 5`
Déclarez une variable nommée `b` et stockez-y la valeur de retour de `can_harvest()` :
`b = can_harvest()`
...
La boucle ci-dessus s'exécute 5 fois parce que `a` est défini sur `5`.
Le `i` dans la boucle `for` est également une variable à laquelle la valeur actuelle de la séquence est automatiquement assignée à chaque itération de la boucle. (Elle ne doit pas forcément s'appeler `i`, vous pouvez lui donner n'importe quel nom de variable valide.)
...
Cela fait la même chose que la boucle for ci-dessus. Nous devons simplement incrémenter `i` manuellement.
