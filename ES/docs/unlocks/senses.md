...
`num_items(item)` devuelve cuántos de un elemento tienes.
Por ejemplo, `num_items(Items.Hay)` devuelve cuánto heno tienes.

`get_entity_type()` y `get_ground_type()` devuelven el tipo de entidad o suelo que está debajo del dron.

Hacer un giro si estás sobre un arbusto:
`if get_entity_type() == Entities.Bush:
    do_a_flip()`

¡La palabra clave `None` también está desbloqueada ahora! `None` es un valor que representa que no hay ningún valor.
