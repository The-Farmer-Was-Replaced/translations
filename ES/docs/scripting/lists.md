# Listas
Las listas son una forma sencilla de almacenar múltiples valores en una sola variable.
...
Los siguientes métodos de lista permiten agregar y eliminar elementos:

`list.append(elem)` agrega un elemento al final de la lista:
...
Si dos variables refieren a la misma lista, los cambios en la lista serán vistos por ambas.

`a = [1,2]
b = a
b.pop()`
`a` y `b` ahora son ambas `[1]`
