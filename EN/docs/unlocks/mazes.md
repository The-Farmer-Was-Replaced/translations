...
The same maze can be solved a maximum of 300 times. This corresponds to 299 relocations. After that, using weird substance on the treasure won't have any effect anymore.
...
<spoiler=show hint 3>Instead of moving in absolute directions like move `East` or move `West` it can be very useful to move in relative directions like "turn right" or "turn left". To do this you need to keep track of which way the drone is currently moving. The drone never actually rotates, but you can still keep a "virtual" rotation in code.
The following index trick is helpful for this:

`directions = [North, East, South, West]
index = 0`

Use `% 4` to make it rotate "around the clock" so after `West` it wraps back to `North`
`# turn right
index = (index + 1) % 4`
